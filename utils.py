...
def calculate_performance_metrics(df, signals, initial_capital=10000.0, risk_per_trade=0.02):
    ...
    try:
        ...
        entries = trades.index[trades['Entry_Exit'] != 0].tolist()
        ...
        metrics = {
            ...
            'recent_trades': get_recent_trades(df, signals, 10),
            'equity_curve': equity_curve[['Equity', 'Drawdown']],
            'monthly_returns': calculate_monthly_returns(strategy_returns),
            'all_trades': trades_df.sort_index(ascending=False).head(50)  # Latest 50 trades
        }
